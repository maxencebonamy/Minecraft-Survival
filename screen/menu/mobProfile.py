...
class MobProfileScreen(Screen):
    ...
    def __init__(self, window):
        ...
        if self.name.distance == -1:
            distance = i18n('unlimited')
        else:
            distance = self.name.distance
        ...
        self.widgets = {
            ...
            'image': Image(self.window, self.cut.getPos(27, 6), self.cut.getSize(19, 19), self.name.image),
            ...
            'health': Attribute(self.window, self.cut.getPos(1, 11), self.cut.getSize(24, 2), ObjectAttribute.HEALTH, self.name.health, plus=level),
            'damage': Attribute(self.window, self.cut.getPos(1, 14), self.cut.getSize(24, 2), ObjectAttribute.DAMAGE, self.name.damage),
            'distance': Attribute(self.window, self.cut.getPos(1, 17), self.cut.getSize(24, 2), ObjectAttribute.DISTANCE, distance),
        }

        y = 21
        for effect, duration in self.name.effects.items():
            if duration == -1:
                duration = i18n('unlimited')
            self.widgets[effect] = Attribute(self.window, self.cut.getPos(1, y), self.cut.getSize(24, 2), effect, duration, 'effect')
            y += 3
