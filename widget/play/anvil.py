...
class AnvilWidget(Widget):
    ...
    def loop(self):
        ...
        if self.item is not None:
            ...
            if self.widgets['upgradeButton'].isSelected() and Input.clic and self.widgets['upgradeButton'].playAnimWhenSelect:
                Rules.GAME.levels[self.item] += 1
                Rules.GAME.inventory[Item.BOOK] -= 1
                self.item = None
                del self.widgets['upgradeButton']
                del self.widgets['outline']
                del self.widgets['title']
                del self.widgets['image']
                del self.widgets['book']
                self.resetItems()
    ...
    def setItem(self, item):
        ...
        self.widgets['book'] = ItemWidget(self.window, self.getPos(36.5, 6), self.getSize(3, 3), Item.BOOK, 1)
        self.widgets['upgradeButton'] = Button(self.window, self.getPos(34.5, 10), self.getSize(7, 2), i18n('upgrade'))
        if Rules.GAME.inventory[Item.BOOK] > 0:
            self.widgets['upgradeButton'].setAnimSelect()
        ...
        for nb, item in enumerate(Item):
            if item != Item.EMPTY:
                if Rules.GAME.inventory[item] == 0:
                    ...
                    if item.enchantable and 0 < Rules.GAME.levels[item] < 3:
                        self.widgets[f'item{item.name}'] = ItemWidget(self.window, self.getPos(*coords[nb]),
                                                                      self.getSize(3, 3), item, 1, Rules.GAME.levels[item]).setAnimSelect()
                    else:
                        self.widgets[f'item{item.name}'] = ItemWidget(self.window, self.getPos(*coords[nb]),
